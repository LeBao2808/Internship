...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (existingSave) {
      return NextResponse.json({ error: "Blog already saved" }, { status: 400 });
    }
    ...
    await saveBlog.save();
    return NextResponse.json({ message: "Blog saved", saveBlog }, { status: 201 });
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 400 });
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (!result) {
      return NextResponse.json({ error: "Saved blog not found" }, { status: 404 });
    }
